...
//    //创蓝接口参数
//    $postArr = array(
//        'account' => 'N062366_N4745204',
//        'password' => 'jQvJHwcPCSd196',
//        //百星接口
...
//}
function newMsg($mobile, $code) {
        vendor('Sms.SmsapiClass');
        //vendor('phpmailer.PHPMailerAutoload'); 
        $uid = 'uccel5555';
        $pwd = 'uccelfy1234';
        $content = "【Gpays】您的验证码" . $code . "如非本人操作,请忽略本短信";
        $api = new \SmsApi($uid, $pwd);
        $result = $api->sendAll($mobile, $content);
        if ($result['stat'] == '100') {
            return 1;
        } else {
            //console_log('发送失败:' . $result['stat'] . '(' . $result['message'] . ')');
            return 0;
        }
    }
