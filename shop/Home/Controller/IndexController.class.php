...
class IndexController extends CommonController {
    ...
    public function index() {
        ...
        $where['userid'] = $userid;
        $uinfo = M('user')->where($where)->field('img_head,userid,user_credit,is_reward,today_releas,releas_rate')->find();
        $uinfo['releas_rate'] = sprintf('%.2f',$uinfo['releas_rate']);
        ...
        if ($uinfo['is_reward'] == 0) {
            ...
            $fengmi_num = M('store')->where(array('uid' => $userid))->getField('fengmi_num');
            $can_get = $can_get > $fengmi_num ? $fengmi_num : $can_get;
        }
...
//                $res_pay = M('store')->where(array('uid' => $userid))->setDec('fengmi_num',$getnums);//每日积分释放余额
                ...
                if ($res_addrs) {
                    ...
                    //添加加速值日志
                    $relog['releas_rate'] = $uinfo['today_releas'];
                    $relog['uid'] = $userid;
                    $relog['add_time'] = time();
                    M('releas_log')->add($relog);
    ...
    public function Bancerecord() {
        ...
        //$where['get_type'] = array(0,2,'or');
        $where['get_type'] = array('in', '0,2,5,6,7');
        //分页
        ...
        foreach ($Chan_info as $k => $v) {
            ...
            //转入转出
            if ($Chan_info[$k]['get_type'] == 0 && $uid == $v['pay_id']) {
                $Chan_info[$k]['get_nums'] = $Chan_info[$k]['get_nums'] - $Chan_info[$k]['get_nums'] * 2;
            }
            if ($uid == $v['pay_id']) {
    ...
    public function Changeout() {
        ...
        if (IS_AJAX) {
            ...
            $tper = $paynums * 20 / 100;
            $eper = $paynums * 80 / 100;
            $eper1 = $paynums * 50 / 100;
            $datapay['cangku_num'] = array('exp', 'cangku_num - ' . $paynums);
            $datapay['fengmi_num'] = array('exp', 'fengmi_num + ' . $eper1);
...
//            $res_get = M('store')->where(array('uid' => $trid))->save($jifen_donums);//转入的人+20%积分
            //执行分销方法
            //$this->Doprofit($trid, $paynums, 1);
            //判断用户等级
            $this->liutong($uid, $trid, $paynums);
    ...
    public function Doprofit($uid, $paynums, $type) {
        ...
        if (count($Lastinfo) > 0) {
            $this->Addreas($uid, $Lastinfo, $paynums, $type); //加速积分释放
        }
    }
...
//            unset($data);
            ...
            if ($add_Dets) {
                //执行分销方法
                //$res_save = $this->Doprofit($uid, $dhnums, 2); //2018年4月10日14:10:35 HH
                //判断用户等级
                $this->duihuan($uid, $dhnums);
                $uChanlev = D('Home/index');
    ...
    public function quick()
    {
        $uid = session('userid');
        $where['uid'] = $uid;
        $traInfo = M('releas_log');
        //分页
        $p = getpage($traInfo, $where, 20);
        $page = $p->show();
        $Chan_info = $traInfo->where($where)->order('id desc')->select();
        //var_dump($Chan_info);exit;
        foreach ($Chan_info as $k => $v) {
            $Chan_info[$k]['add_time'] = date('Y-m-d', $v['add_time']);
            $Chan_info[$k]['add_time1'] = date('H:i:s', $v['add_time']);
        }
        if (IS_AJAX) {
            if (count($Chan_info) >= 1) {
                ajaxReturn($Chan_info, 1);
    ...
    public function Turncords2()
    {
        ...
        $Chan_info = $traInfo->where($where)->find();
            $getPay = M('user')->where(array('userid' => $Chan_info['get_id']))->Field('username,mobile,userid')->find();
            $getPerson = M('user')->where(array('userid' => $Chan_info['pay_id']))->Field('username,mobile')->find();
        $this->assign('getPay', $getPay);
        $this->assign('getPerson', $getPerson);
        $this->assign('Chan_info', $Chan_info);
        $this->assign('uid', $uid);
