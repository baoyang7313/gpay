...
//    }
    ...
    {
        $uid = session('userid');
        $uinfo = M('user')->where(array('userid' => $uid))->field('username,is_dailishang,userid,img_head,use_grade,user_credit')->find();
...
//        $u_ID = M('user')->where(array('userid'=>$userid))->getField('mobile');
    ...
    function team() {
        ...
        $data['count']=M('user')->where($map)->count();  //团队人数
        $user=M('user a');
        //$data['yesum']=$user->where($map)->join('ysk_store b on a.userid=b.uid')->sum('b.cangku_num');
        $data['jfsum']=$user->where($map)->join('ysk_store b on a.userid=b.uid')->sum('b.fengmi_num'); //积分
        $map['use_grade']=0;
        $data['zchy']=M('user')->where($map)->count();  //注册会员
        $map['use_grade']=1;
        $data['pthy']=M('user')->where($map)->count();  //普通会员
        $map['use_grade']=2;
        $data['viphy']=M('user')->where($map)->count(); //黄金VIP
        $map['use_grade']=3;
        $data['hjvip']=M('user')->where($map)->count(); //铂金VIP
        $map['use_grade']=4;
        $data['bjvip']=M('user')->where($map)->count(); //砖石VIP
