 ...
 * @return [type]           [json数据]
 */
function ajaxEcho($message, $status = 0, $url = '', $extra = '') {
    // 返回JSON数据格式到客户端 包含状态信息
    header('Content-Type:application/json; charset=utf-8');
    $result = array(
        'message' => $message,
 ...
 * $type=0志出订单
 */
function matching($uid, $nums, $type=0) {
    $where['pay_state']=0;
    $where['pay_nums']=$nums;
    if($type==0){
        $where['trans_type']=1;
        $where['payin_id']=array('neq',$uid);
    }else{
        $where['trans_type']=0;
        $where['payout_id']=array('neq',$uid);
    }
    $order=M('trans')->where($where)->order('id asc')->find();
    return $order;
}
