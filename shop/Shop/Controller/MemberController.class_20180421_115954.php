...
class MemberController extends CommonController
{
	...
	public function allorder()
	{
		...
		if ($s) {
			$orderList = M("order")->where(array("uid" => $uid, "is_show" => 1, "status" => $s))->order('order_id desc')->select();
		} else {
			$orderList = M("order")->where(array("uid" => $uid, "is_show" => 1))->order('order_id desc')->select();
		}
	...
	public function mine()
	{
		...
		$member = M("user")->where(array("userid" => $uid))->find();
		/*账户积分*/
		$memberjifens = M('store')->where(array('uid' => $uid))->find();
	...
	{
		$orderid = I('order_id');
		$orderinfo = M('order_detail')->where(array('order_id'=>$orderid))->select();
	...
	/*		var_dump($userid);die;*/
			...
			//用户信息
			$userInfo = M("user")->where(array("member_id"=>$userid))->find();
			...
			$data['uid'] = $userid;
			$data['account'] =$userInfo['mobile'];
			$data['username'] =$userInfo['username'];
			$data['type'] = $type;
			$data['realname'] =$realname;
			$data['idcard'] =$idcard;
			$data['phone'] = $phone;
			$data['up_idcard'] =$data['app_idcard_front'];
	        $data['down_idcard'] =$data['app_idcard_back'];
			$data['hand_idcard'] =$data['app_idcard_hands'];
			$data['licence'] =$data['app_blicense'];
			$data['status'] =0;
			$data['time'] =time();
			$data['datestr'] =date("Ymd");

			$isAdd = M("verify_list")->add($data);
